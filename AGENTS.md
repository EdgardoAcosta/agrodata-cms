# QuickStock - Agent Documentation
...
- **Styling**: Bootstrap (or tailwind) + Custom SCSS
- **QR/Barcode Scanner**: vue-qrcode-reader 5.7.3
...
- Real-time QR code and barcode detection using device camera
- Supports multiple barcode formats: `qr_code`, `code_128`, `ean_13`, `upc_a`, `upc_e`
- Visual feedback with corner detection outline (blue stroke)
- Camera controls: Start, Pause, Resume
- Processing states with overlay notifications
- It must be a module capable of being imported by any component

### 2. Scan History

- Recent scans displayed in a list
- Stores up to 12 most recent items
- Shows: raw value, format type, source (live), timestamp
- Auto-updates as new codes are scanned

### 3. PWA Configuration

- Service worker with auto-update
- Offline-ready architecture
- Dev service worker disabled to avoid routing conflicts
- Custom theme colors: primary `#0f172a`, background `#0b1120`
- SVG icon support with maskable variants

### 4. Loading States

- `LoadingOverlay` component for async operations
- Backdrop blur effect
- Disables page scroll when visible
- Auto-cleanup on unmount

### 5. Authentication

- Using the @sidebase/nuxt-auth module for secure authentication
...
- Camera access requires secure context (HTTPS/localhost)
- Service worker support for offline functionality
- Tested formats may vary by device/browser
