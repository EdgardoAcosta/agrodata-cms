  ...
  <v-container fluid class="pa-4">
    <v-row class="mb-4">
      ...
      <v-col cols="12" md="auto" class="d-flex flex-wrap ga-2 align-center">
        <v-text-field v-model="search" density="compact" hide-details variant="outlined" placeholder="Buscar"
          style="width: 220px" />
        <v-select v-model="categoryFilter" :items="categoryOptions" item-title="label" item-value="value"
          density="compact" hide-details variant="outlined" placeholder="Categoría" clearable
          style="max-width: 180px" />
        <v-select v-model="labelFilter" :items="labelOptions" item-title="label" item-value="value" density="compact"
          hide-details variant="outlined" placeholder="Etiqueta" clearable style="width: 180px" />
        <v-btn color="primary" density="comfortable" @click="openCreate">Nuevo producto</v-btn>
      </v-col>
    </v-row>

    <v-card>
      <v-card-title class="d-flex justify-space-between align-center">
        <span class="text-caption text-medium-emphasis">{{ totalItems }} productos</span>
      ...
      <v-data-table-server v-model:items-per-page="itemsPerPage" v-model:page="page" v-model:sort-by="sortBy"
        :headers="headers" :items="products" :items-length="totalItems" :loading="loading" class="elevation-0"
        @update:options="loadItems">
        <template #item.name="{ item }">
          <div class="font-weight-medium">{{ item.name }}</div>
          <div class="text-caption text-medium-emphasis">{{ item.shortDescription }}</div>
          <div class="text-caption text-medium-emphasis" style="font-size: 0.7rem">{{ item.slug }}</div>
        </template>
        ...
        <template #item.price="{ item }">
          <div class="font-weight-medium">{{ formatCurrency(item.price, item.currency) }}</div>
          <div class="text-caption text-medium-emphasis">{{ item.inStock ? 'En stock' : 'Sin stock' }}</div>
        </template>
        ...
        <template #item.actions="{ item }">
          <div class="d-flex ga-2 justify-end">
            <v-btn size="small" variant="tonal" color="primary" @click="openEdit(item)">Editar</v-btn>
            <v-btn size="small" variant="text" color="error" @click="confirmDelete(item)">Eliminar</v-btn>
          </div>
        </template>
        <template #no-data>
          <div class="py-4 text-center text-medium-emphasis">No hay productos con los filtros actuales</div>
        </template>
      </v-data-table-server>
    </v-card>
...
const { data: categoryData } = useFetch<{ data: Category[] }>('/api/categories')
const { data: labelData } = useFetch<{ data: Label[] }>('/api/labels')

const categoryOptions = computed(() => (categoryData.value?.data || []).map((cat) => ({ label: cat.name, value: cat.id })))
const labelOptions = computed(() => (labelData.value?.data || []).map((label) => ({ label: label.name, value: label.id })))
...
const formatCurrency = (value: number, currency: string) =>
  new Intl.NumberFormat('es-MX', { style: 'currency', currency }).format(value || 0)
...
const loadItems = async () => {
  ...
  try {
    ...
    const response = await $fetch<{ data: Product[]; total: number }>('/api/products', {
      params: {
        page: page.value,
...
const handleSubmit = async () => {
  ...
  try {
    if (editingId.value) {
      await $fetch(`/api/products/${editingId.value}`, { method: 'PATCH', body: { ...form } })
    } else {
