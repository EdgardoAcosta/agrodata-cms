    ...
    <v-card-text>
      ...
      <div class="d-md-none mb-3">
        <v-btn
          color="primary"
          block
          :disabled="cameraPrimaryDisabled"
          :loading="loading"
          @click="handleCameraPrimary"
        >
          {{ cameraPrimaryLabel }}
        </v-btn>
      ...
      <div class="qr-shell rounded-lg overflow-hidden">
        ...
        >
          ...
          <div class="text-caption">
            Start the camera to request permission and begin scanning.
          </div>
        </div>
        <QrcodeStream
          v-else
          ...
          :torch="torchEnabled && torchSupported"
          @detect="onDetect"
    ...
    <v-card>
      ...
      <v-card-text>
        <v-select
          ...
          density="compact"
          variant="outlined"
          :hint="`${availableCameras.length} detected`"
          persistent-hint
        >
          <template #item="{ props, item }">
            <v-list-item
              ...
              :title="
                item.raw.label ||
                `Camera ${availableCameras.indexOf(item.raw) + 1}`
              "
            />
          </template>
        </v-select>
        ...
        <v-switch
          v-model="torchEnabled"
          :disabled="!torchSupported"
          :label="`Flashlight ${torchSupported ? '' : '(not supported)'}`"
          color="primary"
          class="mt-3"
        />
      </v-card-text>
      ...
      <v-card-actions class="d-flex justify-end">
        <v-btn variant="outlined" @click="showSettingsModal = false"
          >Close</v-btn
        >
      </v-card-actions>
    </v-card>
...
const formats = ref<BarcodeFormat[]>(
  props.formats || ["qr_code", "code_128", "ean_13", "upc_a", "upc_e"],
);
...
const validationPending = computed(
  () => isValid.value === undefined && cameraPaused.value,
);
...
const onDetect = async (result: { format: string; rawValue: string }[]) => {
  ...
  decodedContent.value = result
    .map((el) => `${el.rawValue} (${el.format})`)
    .join(" • ");
  isValid.value = true;
...
function paintOutline(
  ...
  for (const detectedCode of detectedCodes) {
    if (!detectedCode?.cornerPoints || detectedCode.cornerPoints.length === 0)
      continue;
